...
function reEditLine(pageId, cover, replacement) {
  ...
  // Re-derive the doc font / coverage-check object the same way smartReplace
  // does (prepareDocFont only ever reads `line.pdf` — cover.replaceTargets[0]
  // IS that same pdf-space target, captured at the original edit's birth).
  // Fire-and-forget: the twin (already showing via draft.fontFamily) is the
  // honest fallback until/unless this resolves.
