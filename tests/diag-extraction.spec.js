...
import {
  DIRECTION_DOT_MIN,
  PERP_TOLERANCE_FACTOR,
  SPACE_GAP_FACTOR,
} from '../js/core/text-lines.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const NASTY = (name) => path.join(__dirname, 'fixtures', 'nasty', name);

// These thresholds come from the production module. This diagnostic used to
// copy them by hand, so changing the real clustering gates could leave its
// arithmetic and logs confidently describing an implementation that no
// longer existed. The former COLUMN_GAP_FACTOR copy was unused and is gone.
