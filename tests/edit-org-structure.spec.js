...
test('subtitle edit under a base CTM bakes natively at the ORIGINAL position', async ({ page }) => {
  ...
  // Increment B; FIXED 2026-07-26, founder-flagged correctness bug): this
  // used to ride on js/v2/app.js's prepareDocFont's async race and flake
  // between 'os2' and 'none' under full-suite load. core/stamp.js's
  // resolveStampFont no longer trusts a lost-race 'none' hint — it resolves
  // AUTHORITATIVELY against the real document on every path (native
  // included), so this is deterministic again: this subtitle's font has an
  // uninformative /BaseFont wrapper (no PDF-level bold/italic/name signal),
  // but its embedded program's own OS/2 table resolves cleanly ('os2', not
  // bold) — the SAME answer every run, draft-time timing no longer matters.
  expect(out.outcome.insert).toEqual({
    path: 'native', reason: 'clean', style_source: 'os2', glyph_shortfall: 0,
  });
